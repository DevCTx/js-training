...
const updateResultsList = () => {
  ...
  resultList.innerHTML = units
    .filter(unit => unit !== unitToConvert.value)
    .map(unit => {
      const converted = convertQuantity(unitToConvert.value)(unit)(quantity).toFixed(2);  
      return `<li>${ingredientName.value}: ${converted} ${unit}</li>`;
    }).join("");
}
